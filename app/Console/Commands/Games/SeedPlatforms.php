...
use App\Jobs\Games\SeedPlatformsJob;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Bus;

class SeedPlatforms extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'igdb:seed-platforms';
    /**
    ...
    public function handle(): void
    {
        try {
            ...
            $chunkNumber = 0;
            do {
                $job = new SeedPlatformsJob($chunkNumber);
                Bus::dispatch($job);
                $chunkNumber++;
            } while (count(FetchPlatformsAction::execute($chunkNumber, 'id asc', ['id'])) > 0);
